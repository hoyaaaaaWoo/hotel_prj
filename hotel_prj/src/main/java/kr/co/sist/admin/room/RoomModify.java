...
public class RoomModify {
	...
	public int UpdateRoomStatus(String statusRNo, String rStatus) throws DataAccessException {
		...
		updateStatus.append("update 	room		").append(" set   r_status=?	").append(" where 	room_no=?");
	...
	public boolean updateRoom(RoomVO roomVO) throws DataAccessException {
		...
		updateRoom.append("	update	 room	").append(
				"	set		r_name=?, 	description=?,	price=?,	bed_type=?, 	max_guest=?,	r_size=?,	")
		...
		int cnt = jt.update(updateRoom.toString(), roomVO.getRoomName(), roomVO.getMainDesc(), roomVO.getPrice(),
				roomVO.getType(), roomVO.getGuestNum(), roomVO.getRoomSize(), roomVO.getChkIn(), roomVO.getChkOut(),
				roomVO.getView(), roomVO.getSpecialServ(), roomVO.getGeneralAmn(), roomVO.getBathAmn(),
				roomVO.getOtherAmn(), roomVO.getMoreInfo(), roomVO.getImg(), roomVO.getRoomNum());
	...
	public boolean deleteImg(RoomVO roomVO) throws DataAccessException {
		...
		if (list.size() == 0) {
			...
			deleteImg.append("delete	from 	images	").append("where	room_no=?");
